...
| file_search | 对已上传文本文件维护倒排索引和 BM25 排序，把命中窗口注入模型上下文，并输出 `file_search_call` / metadata / input_items 证据链 |
| Computer executor | `computer_use` / `computer_use_preview` 转换为 `local_computer`；启用 `DEECODEX_COMPUTER_EXECUTOR=playwright` 后可执行 `open_url`、`screenshot`、`click`、`type`、`keypress`、`scroll` 并回填 `computer_call_output`；`browser-use` 暂无本地 bridge 时返回明确失败 output |
