...
| Computer bridge | `computer_use` / `computer_use_preview` 转换为 `local_computer` bridge；上游返回 local computer 调用时映射为 Responses `computer_call`，可用 `CODEX_RELAY_ALLOWED_COMPUTER_DISPLAYS` 限制 display |
| MCP bridge | `mcp` / `remote_mcp` 转换为 `local_mcp_call` bridge；上游返回 local MCP 调用时映射为 Responses `mcp_tool_call`，可用 `CODEX_RELAY_ALLOWED_MCP_SERVERS` 限制 server |
