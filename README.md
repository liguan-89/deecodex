...
| MCP bridge | `mcp` / `remote_mcp` 转换为 `local_mcp_call` bridge；上游返回 local MCP 调用时映射为 Responses `mcp_tool_call`，可用 `DEECODEX_ALLOWED_MCP_SERVERS` 限制 server，并已预留 `DEECODEX_MCP_EXECUTOR_CONFIG` 执行器配置 |
...
| `DEECODEX_MCP_EXECUTOR_CONFIG` | MCP server JSON 对象/数组，或 JSON 文件路径 | `""` |
