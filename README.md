...
| Files API | 支持上传、列表、读取 metadata、读取内容、删除；当前为内存存储，服务重启后丢失 |
| Vector stores | 支持本地 vector store、文件关联和 file batch 壳层；用于约束本地 file_search 范围 |
| file_search | 对已上传文本文件做轻量本地检索，把命中内容注入模型上下文，并把结果写入 metadata；后续补齐可选 `file_search_call` output item |
| Computer bridge | `computer_use` / `computer_use_preview` 转换为 `local_computer` bridge；上游返回 local computer 调用时映射为 Responses `computer_call` |
| MCP bridge | `mcp` / `remote_mcp` 转换为 `local_mcp_call` bridge，为后续真实 MCP executor 保留结构化入口 |
