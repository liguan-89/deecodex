...
你正在 **deecodex-gui 父区** 工作。你可以修改共享层文件和履行父区职责，排查 bug 时通读全项目代码。负责：

- **合入功能分支：** `git merge 功能/<分区名>`
- **同步 worktree：** `for b in ...; do git -C "功能/$b" merge deecodex-gui; done`
- **版本号管理：** 升级 `Cargo.toml`、`deecodex-gui/Cargo.toml`、`deecodex-gui/tauri.conf.json`、`VERSION` 四处版本号
- **打 tag 发版：** `git tag v<版本号> && git push deecodex-new deecodex-gui && git push origin v<版本号>`
- **修改 GUI 共享层文件**（见下方"GUI 共享层"章节）时在此进行，不替分区 agent 改业务文件
