...
- Local executor settings are parsed in `executor.rs` and stored on `AppState`; they are default-disabled and only define the safe configuration boundary for later MCP/browser execution.
...
| `executor` | ~250 | Local computer/MCP executor configuration model and parser |
