// 消息处理管道：授权检查 → 上下文装配 → LLM 调用 → Markdown 过滤 → 发送回复
import type { WeixinMsgContext, ChatMessage, WeixinPluginConfig } from "./types.js";
import { getConfig, getDataDir, sendNotification, sendRequest, getLlmBaseUrl } from "./rpc-client.js";
import { sendMessage, sendTyping, getUploadUrl, uploadToCdn } from "./ilink.js";
import { aesEcbEncrypt, extToMime, mimeToExt } from "./media.js";
import * as fs from "node:fs";
import * as path from "node:path";

// ── 授权检查 ──

function checkAuthorization(ctx: WeixinMsgContext): boolean {
  const cfg = getConfig();

  // open 模式：不检查
  if (cfg.dm_policy === "open") return true;

  // whitelist 模式：检查白名单
  const allowedUsers = cfg.allowed_users || [];
  if (allowedUsers.length === 0) return true; // 空白名单 = 允许所有
  return allowedUsers.includes(ctx.sender_id);
}

// ── 上下文管理 ──

function loadContext(chatId: string, maxMessages: number): ChatMessage[] {
...
function saveContext(chatId: string, messages: ChatMessage[]): void {
  const dataDir = getDataDir();
...
async function sendTextReply(token: string, chatId: string, text: string): Promise<void> {
...
export async function processMessage(
  ...
  try {
    ...
    const responseObj = result as { content?: string };
