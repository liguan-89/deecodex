// 长轮询网关：getupdates 循环
import type { WeixinMessage, WeixinMsgContext, WeixinPluginConfig } from "./types.js";
...
export async function startGateway(accountId: string): Promise<void> {
...
export async function stopGateway(accountId: string): Promise<void> {
...
export function isGatewayRunning(accountId: string): boolean {
  return gateways.has(accountId) && gateways.get(accountId)!.running;
}

// ── 长轮询循环 ──

async function pollLoop(state: GatewayState): Promise<void> {
  sendNotification("log", {
    level: "info",
...
function loadBuf(accountId: string): string {
  try {
...
function saveBuf(accountId: string, buf: string): void {
  try {
