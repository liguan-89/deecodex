...
import { getConfig, getDataDir, sendNotification } from "./rpc-client.js";
...
export function aesEcbDecrypt(buffer: Buffer, keyHex: string): Buffer {
...
export function aesEcbEncrypt(buffer: Buffer, keyHex: string): Buffer {
  const key = Buffer.from(keyHex, "hex");
...
export async function transcodeSilkToWav(silkPath: string): Promise<string | null> {
