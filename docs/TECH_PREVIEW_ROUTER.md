...
- 通过本地路由层接管模型请求，将官方模型请求映射到第三方或中转模型。
...
5. 将单一模型请求映射到一个第三方 Responses 账号。
6. 增加路由诊断：登录态存在、provider 命中、模型映射、工具降级原因。

## 后续能力

- 官方模型 passthrough：保留官方模型真实调用能力。
- 中转模型接管：将 `gpt-5.x` 等官方模型映射到第三方模型。
...
- 执行账号从同一个账号池中选择，不再固定使用当前活跃账号：
  ...
  - Chat / Anthropic 翻译端点必须有请求模型映射；Responses 直连和 Codex 官方端点允许原模型名透传。
- 路由执行结果会写回账号运行态：
  - 成功请求清理当前模型的失败/冷却状态。
...
- 官方 Codex 账号的路由身份已经拆分：
  ...
  - 第三方 / 直连执行账号仍按同池、模型映射、能力矩阵、健康度和冷却状态参与候选。
